...
W_AGGRESSION_BONUS = 2.0        # NOVO: Bônus garantido por ATIRAR em modo de combate.
W_DAMAGE_DEALT_BONUS = 15.0     # Aumentado: torna o dano efetivo ainda mais recompensador.
W_EVASION_BONUS = 3.0           # Reduzido: A evasão ainda é boa, mas não deve superar a agressão.
...
def initialize_game():
    ...
    actions = [
        ...
        [0, 0, 0, 0, 0, 1, 1],  # 4: Atirar e Strafe para Esquerda (Ação de combate)
        [0, 0, 0, 0, 0, 0, 1],  # 5: Atirar e Strafe para Direita (Ação de combate)
    ]
    return game, actions
...
def calculate_robust_fitness(game, individual, actions):
    ...
    for _ in range(NUM_EVAL_RUNS):
        ...
        for action_index in individual["genome"]:
            ...
            if state is None: continue # Garante que o estado não é nulo
            ...
            if agent_state == "CLEAR_NAVIGATION":
                ...
                # Lógica de incentivo ao combate Agressivo
                if is_shooting:
                    # Bônus garantido por tentar atirar
                    total_reward_for_episode += W_AGGRESSION_BONUS
                    
                    # Recompensa por dano efetivo
                    ...
                    last_damage_dealt = current_damage_dealt

                # Lógica de evasão (agora menos dominante)
                ...
                # Bônus por kill continua o mesmo
