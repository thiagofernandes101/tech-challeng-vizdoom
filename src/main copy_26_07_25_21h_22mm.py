...
W_COMBAT_DAMAGE_PENALTY = -10.0  # Reduzido para permitir mais agressividade
NUM_EVAL_RUNS = 1                       # Número de partidas para avaliar cada indivíduo
...
W_TIME_PENALTY = -0.1            # Penalidade a cada tick para criar um senso de urgência.

W_AGGRESSION_BONUS = 7.0        # Aumentado para incentivar mais tiros em combate
W_DAMAGE_DEALT_BONUS = 20.0     # Aumentado para incentivar mais dano
W_EVASION_BONUS = 2.0           # Mantido, mas com menor peso
...
W_COMBAT_KILL_BONUS = 500.0             # Aumentado para priorizar abates
...
STAGNATION_LIMIT = 100 # <-- MUDANÇA CRÍTICA
...
def initialize_game():
    ...
    game.set_available_buttons([
        ...
        vzd.Button.MOVE_LEFT,   # Strafe Esquerda
        vzd.Button.MOVE_RIGHT,  # Strafe Direita
    ])
    game.init()
    # actions = [
    #     [1, 0, 0, 0, 0, 0, 0],  # 0: Atirar
    #     [0, 1, 0, 0, 0, 0, 0],  # 1: Mover para frente
    #     [0, 0, 0, 1, 0, 0, 0],  # 2: Virar para esquerda
    #     [0, 0, 0, 0, 1, 0, 0],  # 3: Virar para direita
    #     [0, 0, 0, 0, 0, 1, 0],  # 4: Strafe Esquerda (corrigido: action 4 era atirar e strafe)
    #     [0, 0, 0, 0, 0, 0, 1],  # 5: Strafe Direita (corrigido: action 5 era atirar e strafe)
    #     [1, 1, 0, 0, 0, 0, 0],  # 6: Atirar e Mover para frente (adição para incentivar avanço em combate)
    #     [1, 0, 0, 0, 0, 1, 0],  # 7: Atirar e Strafe Esquerda (reintroduzido como nova ação)
    #     [1, 0, 0, 0, 0, 0, 1],  # 8: Atirar e Strafe Direita (reintroduzido como nova ação)
    #     [0, 0, 0, 0, 0, 0, 0]   # 9: Parar / No-op (útil para períodos de observação ou ajustes finos)
    # ]
    actions = [list(action) for action in product([0, 1], repeat=7)]
    return game, actions
...
def calculate_robust_fitness(game, individual, actions):
    ...
    for _ in range(NUM_EVAL_RUNS):
        ...
        last_kills = 0
        last_damage_dealt = 0 # NOVO: Rastrear dano causado
        ...
        for action_index in individual["genome"]:
            ...
            # APLICA A PENALIDADE POR TEMPO A CADA TICK
            ...
            is_shooting = action_to_perform[0] == 1
            ...
            if agent_state == "CLEAR_NAVIGATION":
            ...
            elif agent_state == "COMBAT" or agent_state == "CAUTIOUS_NAVIGATION":
                # Lógica de incentivo ao combate Agressivo (aplicada em ambos os estados com inimigos,
                # para garantir que ele esteja sempre "pronto" para o combate).
                # O Cautious_Navigation pode ser considerado um "pré-combate"
                
                if is_shooting:
                    # Bônus garantido por tentar atirar em combate ou perto de inimigos
                    total_reward_for_episode += W_AGGRESSION_BONUS
                    
                    # Recompensa por dano efetivo causado
                    current_damage_dealt = game.get_game_variable(vzd.GameVariable.DAMAGECOUNT)
                    damage_delta = current_damage_dealt - last_damage_dealt
                    if damage_delta > 0:
                        total_reward_for_episode += W_DAMAGE_DEALT_BONUS * damage_delta
                    last_damage_dealt = current_damage_dealt # Atualiza o dano causado para a próxima iteração

                # Lógica de evasão / penalidade por dano recebido
                health_lost = last_health - game.get_game_variable(vzd.GameVariable.HEALTH)
                if health_lost <= 0: 
                    # Se não perdeu vida, recompensa pela "evasão" ou boa gestão de combate
                    total_reward_for_episode += W_EVASION_BONUS
                else: 
                ...
                if current_kills > last_kills: 
                    total_reward_for_episode += W_COMBAT_KILL_BONUS
                last_kills = current_kills # Atualiza o número de kills

                # Se estiver em CAUTIOUS_NAVIGATION (inimigos longe mas presentes), ainda recompensa o progresso
                # mas com um multiplicador menor, para que o foco principal não seja só o objetivo.
                # Removido para simplificar, como mencionado nos parâmetros. A lógica principal é
                # COMBAT vs. CLEAR_NAVIGATION.
                if agent_state == "CAUTIOUS_NAVIGATION":
                    total_reward_for_episode += W_CAUTIOUS_PROGRESS_MULTIPLIER * W_GOAL_PROGRESS * distance_reduction
        ...
        if not game.is_player_dead() and (game.is_episode_finished() or np.linalg.norm(GOAL_POSITION - player_pos) < 50):
